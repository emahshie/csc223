...
- Graphs are used to model a situation where things are related to each other in pairs (ie family trees or transportation networks)
...
- Degree of a node: the number of edges containing the node, denoted deg(u)
...
- Simple cycle: a cycle with no repeated edges and vertices except the first and last
...
- Labelled/weighted graph: every edge is assigned to some data (labelled), every edge is assigned some weight or length w(e) that is the cost of traversing the edge (weighted)
...
- Source: a node with a positive out-degree but a zero in-degree 
- Sink: a node with a positive in-degree but a zero out-degree 
